...
def connect_to_db():
    try:
        ...
        print(f"Connecting to MySQL database with the following credentials:")
        print(f"Host: {host}")
        print(f"User: '{user}'")
...
def custom_data_query(request):
    ...
    payload = f"Generate only the SQL query with no additional text based on this structure. DO NOT INCLUDE ANY NEWLINES OR SPECIAL CHARATERS IN THE RESPONSE. The database is called patient. Do not wrap it in code formatting, just plaintext. If you are not sure of what query and are tempted to just do SELECT * FROM patient, you may also just respond with the string '123'.: patient_id, name, address, phone_number, age, race, gender, insurance, smoking, physical_activity, alcohol, support_system. Query: {query}"
...
def handle_query():
    ...
    # user_message = request.json['messages'][0]['text']

    # if not user_message.startswith("!") or :
    #     return custom_data_query(request)
    # else:
    #     return demo_convo(request)
    ...
    try:
        ...
        if count == 5:
            ...
            <div style="display: flex; flex-direction: column; align-items: flex-start;">
                ...
                <img src="https://cdn.discordapp.com/attachments/1302056188887699499/1302255914652270642/6.png?ex=672773d4&is=67262254&hm=f8393059dbcbbdb9ebca8f31fe0c4630f7ce6c6d9095c92b50b7e840bf92e082&" alt="Reference PDF" height="300px">
                <p>Also, here is a pdf for reference.</p>
            </div>
            """
